...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check live Google Drive Singer tap contract.")
    parser.add_argument(
        "--artifact-dir",
        type=Path,
        default=Path(".local/test-artifacts/google-drive-live-contract"),
        help="Ignored local directory for raw landing files.",
    )
    args = parser.parse_args()

    report = check_google_drive_live_contract(args.artifact_dir)
    print(json.dumps(report, indent=2, sort_keys=True))
    return 0 if report["status"] == "ok" else 1


def check_google_drive_live_contract(artifact_dir: Path) -> dict[str, Any]:
