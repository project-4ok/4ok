...
def configure_observability_from_env() -> ObservabilityConfig | None:
    if os.environ.get("GCB_OBSERVABILITY_ENABLED", "").lower() not in {"1", "true", "yes"}:
        return None
...
def emit_observability_smoke(
    *,
    service_name: str = "gcb-local-smoke",
    endpoint: str = DEFAULT_OTLP_ENDPOINT,
) -> dict[str, object]:
    config = configure_observability(service_name=service_name, endpoint=endpoint)
    tracer = trace.get_tracer(__name__)
    with tracer.start_as_current_span("gcb.observability_smoke") as span:
        span.set_attribute("gcb.signal", "safe-smoke")
        span.set_attribute("gcb.sensitive_payload_exported", False)
        logging.getLogger(__name__).info(
            "gcb observability smoke",
            extra={"gcb_signal": "safe-smoke"},
        )
