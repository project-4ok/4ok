...
def dagster_status_report(
    *,
    ...
    repo = _discover_gcb_repository(repository_nodes if isinstance(repository_nodes, list) else [])
    runs_data = graphql_call(
        dagster_url,
        _DAGSTER_RUNS_QUERY,
        {"pipelineName": "gcb_hourly_live_backfill", "limit": 5},
    )
...
def _discover_gcb_repository(nodes: list[object]) -> dict[str, object]:
    if not nodes:
        return {}
    ...
    def match(node: object) -> dict[str, object] | None:
        ...
        if isinstance(location, dict) and location.get("name") == "gcb_pipeline":
            return node
        pipelines = [
            item.get("name") for item in node.get("pipelines", []) if isinstance(item, dict)
        ]
        if "gcb_hourly_live_backfill" in pipelines:
            return node
        return None
