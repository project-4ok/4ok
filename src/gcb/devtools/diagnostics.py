...
PROJECT_LOG_ROOTS = (".local", "logs", "var/log/gcb")
...
def _docker_pipeline_check(
    ...
    try:
        ...
        return _check(
            ...
            ["uv run gcb-dev pipeline-ps", "uv run gcb-dev pipeline-up"],
        )
...
def _dagster_check(dagster_url: str) -> dict[str, object]:
    try:
        ...
        return _check(
            ...
            ["uv run gcb-dev pipeline-up", "uv run gcb-dev pipeline-ps"],
        )
    return _check(
        "dagster",
    ...
    *,
    ...
    if not database_url and not resolved_state.exists():
        return _check(
            "database",
            "skipped",
            {"reason": "no GCB_DATABASE_URL and no local state database"},
...
def _raw_store_check(project_root: Path, *, raw_store_path: Path | None) -> dict[str, object]:
    ...
    if not path.exists():
        return _check(
            ...
            ["uv run gcb health --raw-store .local/raw"],
        )
    return _check(
        "raw_store",
    ...
    *,
    ...
    if not database_url and not resolved_state.exists():
        return _check(
            ...
            ['uv run gcb search-state "diagnostic health probe" --state .local/context.sqlite'],
        )
    try:
