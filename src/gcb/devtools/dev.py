...
from gcb.devtools.dagster_status import dagster_status_report
from gcb.devtools.diagnostics import DEFAULT_ERROR_WINDOW_SECONDS, agent_diagnostics
...
def build_plan(action: str, extra_args: Sequence[str]) -> list[DevStep]:
    ...
    if action == "pipeline-up":
        return [_pipeline_up()]
    if action == "stack-up":
        return [_app_up(), _observability_up(), _pipeline_up()]
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="gcb-dev")
    ...
    diagnostics_parser.add_argument("--database-url", default=os.environ.get("GCB_DATABASE_URL"))
    ...
    compose_env_parser = subparsers.add_parser(
        "compose-env",
        help="Print redacted compose/runtime environment resolved by gcb-dev.",
    )
    ...
    logs_status_parser = subparsers.add_parser(
        "logs-status",
        help="Summarize Loki Docker log aggregation status and useful GCB LogQL queries.",
    )
...
def _format_check() -> DevStep:
    return DevStep("format-check", ("uv", "run", "python", "-m", "gcb.devtools.dev", "format"))


def _lint() -> DevStep:
    return DevStep("lint", ("uv", "run", "python", "-m", "gcb.devtools.dev", "lint"))
...
def _pytest(extra_args: Sequence[str]) -> DevStep:
    ...
    return DevStep("pytest", ("uv", "run", "python", "-m", "gcb.devtools.dev", "test-tracked"))


def _goal_audit() -> DevStep:
    return DevStep("goal-audit", ("uv", "run", "gcb", "goal-audit"))
...
def _compose_local_env() -> dict[str, str]:
    ...
    env.setdefault("GCB_IMAGE_TAG", _git_short_head(default="local-check"))
    env.setdefault(
        "GCB_DATABASE_URL",
...
def compose_env_report(*, redact: bool = True) -> dict[str, object]:
    ...
    return {
        ...
        "env": rendered,
        "usage": {
            "compose_config": "uv run gcb-dev compose-config",
            "app_up": "uv run gcb-dev app-up",
            "observability_up": "uv run gcb-dev observability-up",
            "pipeline_up": "uv run gcb-dev pipeline-up",
            "stack_up": "uv run gcb-dev stack-up",
            "pipeline_ps": "uv run gcb-dev pipeline-ps",
        },
    }
...
def logs_status_report(
    *,
    ...
    return {
        "status": "ok" if counts["all_gcb"]["entries"] > 0 else "no_recent_logs",
        "loki_url": loki_url,
        "since_seconds": since_seconds,
        "compose_services": compose_services_response.get("data", []),
        "queries": queries,
        "counts": counts,
        "grafana_dashboard": "http://127.0.0.1:3000/d/gcb-local-runtime-logs/gcb-local-runtime-logs",
    }
