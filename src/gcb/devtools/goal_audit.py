...
from gcb.runtime.active_surface import (
    ACTIVE_CLI_COMMANDS,
...
PLAN_GCB_PROOF_PATTERN = re.compile(r"Proof: `uv run gcb ([a-z0-9-]+)(?:\s|`)")
...
def _check_plan_proof_commands_use_active_cli(project_root: Path) -> GoalAuditCheck:
    content = _read(project_root / "docs/plan.md")
    commands = PLAN_GCB_PROOF_PATTERN.findall(content)
...
def _check_cli_active_surface(project_root: Path) -> GoalAuditCheck:
    content = _read_existing(
        project_root / "src/gcb/cli.py",
        *sorted((project_root / "src/gcb/cli_parts").glob("parser*.py")),
    )
...
def _check_compose_uses_pinned_internal_images(project_root: Path) -> GoalAuditCheck:
    ...
    required = ("4ok-app:${GCB_IMAGE_TAG:?set GCB_IMAGE_TAG}",)
...
def _check_compose_app_requires_database_url(project_root: Path) -> GoalAuditCheck:
    ...
    required_database_url = "GCB_DATABASE_URL: ${GCB_DATABASE_URL:?set GCB_DATABASE_URL}"
    ...
    if "gcb_dev_password" in content:
        return GoalAuditCheck(
            "compose_app_requires_database_url",
            "failed",
...
def _check_systemd_uses_env_file_for_runtime_secrets(project_root: Path) -> GoalAuditCheck:
    ...
    for path in service_paths:
        content = _read(path)
        relative_path = str(path.relative_to(project_root))
        if "EnvironmentFile=/etc/gcb/gcb.env" not in content:
...
def _check_alert_guidance_fields(project_root: Path) -> GoalAuditCheck:
    runtime_files = (
        project_root / "src/gcb/runtime/dashboard.py",
        project_root / "src/gcb/runtime/acceptance.py",
    )
