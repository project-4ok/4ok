...
def embedding_provider() -> str:
    configured = os.environ.get("GCB_EMBEDDING_PROVIDER")
    if configured:
...
def embedding_dimensions() -> int:
    configured = os.environ.get("GCB_EMBEDDING_DIMENSIONS")
    if configured:
...
def _hash_embed_text(text: str, *, dimensions: int) -> list[float]:
    ...
    for term in _terms(text):
        digest = blake2b(term.encode(), digest_size=4, person=b"gcbembed").digest()
...
def _openai_embed_text(text: str, *, dimensions: int) -> list[float]:
    ...
    if not api_key:
        raise RuntimeError("OPENAI_API_KEY is required when GCB_EMBEDDING_PROVIDER=openai")
    model = os.environ.get("GCB_OPENAI_EMBEDDING_MODEL", OPENAI_EMBEDDING_MODEL)
    endpoint = os.environ.get("GCB_OPENAI_EMBEDDING_URL", "https://api.openai.com/v1/embeddings")
    payload = {
        "model": model,
        "input": text,
