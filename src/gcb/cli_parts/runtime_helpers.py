...
from gcb.observability import configure_observability, configure_observability_from_env
from gcb.runtime.webhooks import WebhookEventInput
...
def _database_url_from_args(args: argparse.Namespace) -> str | None:
    ...
    if (
        (
            ...
            or getattr(args, "state", None) != Path(".gcb-state.sqlite")
        )
        and database_url is not None
        and database_url == os.environ.get("GCB_DATABASE_URL")
    ):
        return None
    return database_url
