...
from gcb.etl.extract.connectors import ConnectorPayloadError, load_gmail_source_records
...
from gcb.governance import SourceChange
from gcb.runtime.source_imports import import_source_records
from gcb.storage.config import RuntimeConfig
...
def _run_imports(args: argparse.Namespace) -> dict[str, object]:
    ...
    with tracer.start_as_current_span("gcb.run_imports") as span:
        try:
            return _run_imports_with_span(args, span)
        except Exception as exc:
            span.set_attribute("gcb.import.status", "failed")
...
def _run_imports_with_span(args: argparse.Namespace, span) -> dict[str, object]:
    ...
    span.set_attribute("gcb.connector.name", args.connector)
    ...
    if args.retry_failed:
        ...
        if retry_plan is None:
            span.set_attribute("gcb.import.status", "skipped")
            span.set_attribute("gcb.import.skip_reason", "no_failed_connector_job_due")
            return {
                "status": "skipped",
                "connector_name": args.connector,
        ...
        if retry_plan.attempt > max_attempts:
            span.set_attribute("gcb.connector.attempt", retry_plan.attempt)
            span.set_attribute("gcb.import.status", "skipped")
            span.set_attribute("gcb.import.skip_reason", "connector_retry_attempts_exhausted")
            return {
                "status": "skipped",
                "connector_name": args.connector,
        ...
        if datetime.fromisoformat(retry_plan.earliest_retry_at) > retry_check_time:
            span.set_attribute("gcb.connector.attempt", retry_plan.attempt)
            span.set_attribute("gcb.import.status", "retry_not_due")
            return {
        ...
        attempt = retry_plan.attempt
    span.set_attribute("gcb.connector.attempt", attempt)
    ...
    if start_result.started is None:
        ...
        span.set_attribute("gcb.import.status", "skipped")
        span.set_attribute("gcb.import.skip_reason", "connector_job_already_running")
        return {
            "status": "skipped",
            "connector_name": args.connector,
    ...
    try:
        ...
        span.set_attribute("gcb.import.status", "invalid")
    ...
    span.set_attribute("gcb.import.status", "succeeded")
    span.set_attribute("gcb.import.record_count", output_state["record_count"])
    span.set_attribute("gcb.import.deleted_record_count", output_state["deleted_record_count"])
    span.set_attribute("gcb.import.restricted_count", output_state["restricted_count"])
    complete_connector_job(
        state.engine,
        job_runs=state.connector_job_runs,
        connector_states=state.connector_states,
