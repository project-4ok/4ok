...
from gcb.cli_parts.runtime_helpers import _config_from_args, _context_state_from_args
...
from gcb.observability import emit_observability_smoke
...
from gcb.runtime.dashboard import operator_dashboard, operator_status
from gcb.runtime.dependency_contracts import dependency_contract_report
from gcb.runtime.operator_live import _dotenv_values, host_database_url, operator_environment
from gcb.runtime.readiness import internal_prod_readiness_report
from gcb.runtime.rebuild import rebuild_retrieval_units
from gcb.runtime.services import runtime_service_boundaries
from gcb.runtime.stage1_acceptance import run_stage1_acceptance
...
def host_operator_database_url(
    *,
    ...
    if compose_database_url := dotenv_env.get("GCB_DATABASE_URL"):
        return host_database_url(compose_database_url)
    if postgres_password := dotenv_env.get("POSTGRES_PASSWORD"):
        return host_database_url(f"postgresql+psycopg://gcb:{postgres_password}@postgres:5432/gcb")
    operator_env = operator_environment(Path("."))
    if compose_database_url := operator_env.get("GCB_DATABASE_URL"):
        return host_database_url(compose_database_url)
    if env_database_url := os.environ.get("GCB_DATABASE_URL"):
