...
class OpenClawSearchTools:
    ...
    def gcb_search_context(self, query: str, *, limit: int = 5) -> dict[str, object]:
...
def call_openclaw_tool(
    ...
    if name != "gcb_search_context":
        raise ValueError(f"unsupported OpenClaw GCB tool: {name}")
    query = arguments.get("query")
    if not isinstance(query, str) or not query.strip():
        raise ValueError("gcb_search_context.query must be a non-empty string")
    limit = arguments.get("limit", 5)
    if not isinstance(limit, int) or limit < 1 or limit > 20:
        raise ValueError("gcb_search_context.limit must be between 1 and 20")
    return tools.gcb_search_context(query, limit=limit)
...
def openclaw_messages_to_source_records(messages: list[OpenClawMessage]) -> list[SourceRecord]:
    ...
    with tracer.start_as_current_span("gcb.openclaw.capture") as span:
        records = [_openclaw_message_to_source_record(message) for message in messages]
        span.set_attribute("gcb.openclaw.message_count", len(messages))
        span.set_attribute("gcb.openclaw.record_count", len(records))
        span.set_attribute(
            "gcb.openclaw.session_count",
