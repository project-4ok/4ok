...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    database_url = args.database_url or env.get("GCB_DATABASE_URL", "")
...
def build_operator_live_dry_run(
    *,
    ...
    return {
        ...
        "gcb_database_url": redacted_database_url(database_url),
...
def run_operator_live(
    *,
    ...
    run_env = dict(env)
    run_env["GCB_PROJECT_ROOT"] = str(project_root)
    run_env["GCB_RAW_LANDING_DIR"] = str(_project_path(project_root, raw_landing))
    run_env["GCB_STATE_PATH"] = str(_project_path(project_root, state_path))
    if database_url:
        run_env["GCB_DATABASE_URL"] = database_url
...
def _load_dagster_definitions(project_root: Path) -> Any:
    ...
    spec = importlib.util.spec_from_file_location("gcb_operator_dagster_definitions", path)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gcb.runtime.operator_live")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--project-root", type=Path, default=Path("."))
    parser.add_argument("--raw-landing", type=Path, default=Path(".local/raw/singer"))
    parser.add_argument("--state", type=Path, default=Path(".local/dagster/gcb-state.sqlite"))
