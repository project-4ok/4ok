...
DEFERRED_MODULE_PREFIXES = frozenset(
    {
        "gcb.governance.deferred_reveal_policy",
        "gcb.governance.reveal",
        "gcb.governance.token_store",
        "gcb.honcho",
        "gcb.evaluation",
        "gcb.retrieval.evidence_baseline",
        "gcb.retrieval.graphiti_episodes",
        "gcb.etl.transform.pii",
        "gcb.etl.transform.tokens",
    }
)


def active_import_paths(project_root: Path) -> tuple[Path, ...]:
    source_root = project_root / "src/gcb"
