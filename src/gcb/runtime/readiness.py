...
def _check_images(services: dict[str, Any]) -> dict[str, object]:
    ...
    if "${GCB_IMAGE_TAG:?set GCB_IMAGE_TAG}" not in app_image:
        problems.append("app image must be tagged by GCB_IMAGE_TAG")
...
def _check_persistent_volumes(
    ...
    required = {
        ...
        "gcb-local": ("app", "/app/.local"),
        "gcb-data": ("app", "/var/lib/gcb"),
    }
    missing: list[str] = []
...
def _check_app_environment(services: dict[str, Any]) -> dict[str, object]:
    ...
    if "${GCB_DATABASE_URL:?set GCB_DATABASE_URL}" not in str(
        app_environment.get("GCB_DATABASE_URL", "")
    ):
        missing.append("GCB_DATABASE_URL must fail fast when missing")
