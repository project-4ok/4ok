...
from gcb.runtime.metric_helpers import source_latest_record_timestamp_metric as _source_metric
from gcb.runtime.retrieval_metrics import (
...
DAGSTER_JOB = "gcb_hourly_live_backfill"
GCB_DAGSTER_LOCATION = "gcb_pipeline"
GCB_DAGSTER_PIPELINE = "gcb_hourly_live_backfill"
GCB_DAGSTER_SCHEDULE = "gcb_hourly_live_backfill_schedule"
GCB_DAGSTER_SENSOR = "gcb_webhook_backlog_sensor"
...
def render_prometheus_metrics(metrics: Sequence[Metric]) -> str:
    lines = ["# HELP gcb_runtime_exporter_up Whether the GCB runtime exporter rendered metrics."]
    lines.append("# TYPE gcb_runtime_exporter_up gauge")
    lines.append("gcb_runtime_exporter_up 1")
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="python -m gcb.runtime.metrics_exporter")
    parser.add_argument("--host", default=os.environ.get("GCB_METRICS_HOST", "0.0.0.0"))
    parser.add_argument("--port", type=int, default=int(os.environ.get("GCB_METRICS_PORT", "9108")))
    parser.add_argument(
        "--dagster-url",
        default=os.environ.get("GCB_DAGSTER_GRAPHQL_URL", "http://127.0.0.1:3001/graphql"),
    )
    parser.add_argument(
        "--state",
        type=Path,
        default=Path(os.environ.get("GCB_STATE_PATH", "/app/.local/dagster/gcb-state.sqlite")),
    )
    parser.add_argument("--database-url", default=os.environ.get("GCB_DATABASE_URL", ""))
    ...
    def app(environ, start_response):
        ...
        try:
            ...
            body = (
                "# HELP gcb_runtime_exporter_up Whether the GCB runtime exporter "
                "rendered metrics.\n"
                "# TYPE gcb_runtime_exporter_up gauge\n"
                "gcb_runtime_exporter_up 0\n"
                f'gcb_runtime_exporter_error{{error_class="{type(exc).__name__}"}} 1\n'
            )
...
def _dagster_metrics(
    ...
    metrics: list[Metric] = []
    repo = _discover_gcb_repository(repo_data)
    if not repo:
        metrics.append(
            (
                "gcb_dagster_repository_discovery_status",
                {"status": "missing"},
                1.0,
            )
        )
    ...
    for schedule in repo.get("schedules", []) if isinstance(repo, dict) else []:
        ...
        metrics.append(
            ("gcb_dagster_schedule_running", {"name": name}, 1 if status == "RUNNING" else 0)
        )
    for sensor in repo.get("sensors", []) if isinstance(repo, dict) else []:
        ...
        metrics.append(
            ("gcb_dagster_sensor_running", {"name": name}, 1 if status == "RUNNING" else 0)
        )
    ...
    for index, run in enumerate(runs):
        ...
        if index == 0:
            metrics.append(
                ("gcb_dagster_latest_run_status", {"job": DAGSTER_JOB, "status": status}, 1)
            )
        if status not in seen_statuses:
            metrics.append(("gcb_dagster_run_status", {"job": DAGSTER_JOB, "status": status}, 1))
        ...
        if index == 0 and _float(run.get("startTime")) and _float(run.get("endTime")):
            metrics.append(
                (
                    "gcb_dagster_run_duration_seconds",
        ...
        for step in run.get("stepStats", []):
            ...
            if index == 0:
                metrics.append(
                    (
                        "gcb_dagster_latest_run_stage_status",
            ...
            if index == 0 and start and end:
                ...
                metrics.append(
                    (
                        "gcb_dagster_step_duration_seconds",
                        {"job": DAGSTER_JOB, "step": step_key, "status": step_status},
                        duration,
                    )
                )
                if step_key == "gcb_retrieval_records":
                    metrics.append(
                        (
                            "gcb_embedding_index_duration_seconds",
    ...
    for step_key, count in sorted(step_failure_counts.items()):
        metrics.append(
            ("gcb_dagster_step_failures_total", {"job": DAGSTER_JOB, "step": step_key}, count)
        )
    metrics.append(
        ("gcb_dagster_last_success_timestamp_seconds", {"job": DAGSTER_JOB}, last_success)
    )
    return metrics
    ...
    with sqlite3.connect(state_path) as connection:
        ...
        if _has_table(connection, "retrieval_records"):
            for row in connection.execute(
                ...
                metrics.append(
                    ("gcb_retrieval_records_total", {"status": row["status"]}, float(row["count"]))
                )
...
def _connector_latest_metrics_from_rows(rows) -> list[Metric]:
    ...
    for row in rows:
        ...
        status = str(row["status"])
        metrics.append(
            ("gcb_connector_latest_run_status", {"connector": connector, "status": status}, 1)
        )
        metrics.append(
            (
                "gcb_connector_latest_finished_timestamp_seconds",
                {"connector": connector},
...
def _first_repo(payload: dict[str, object]) -> dict[str, object]:
    return _discover_gcb_repository(payload.get("data", {}).get("repositoriesOrError", {}))


def _discover_gcb_repository(payload: dict[str, object]) -> dict[str, object]:
