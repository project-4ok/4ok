...
from gcb.etl.extract.source_records import SourceRecord, email_message_to_source_record
from gcb.etl.load.context_objects import canonical_object_rows, entity_link_rows
from gcb.etl.load.retrieval_records import retrieval_record_rows
from gcb.etl.load.source_changes import (
    SourceChange,
    SourceChangeTables,
...
from gcb.retrieval.search import SearchResult, snippet_for, source_record_search_rows
from gcb.retrieval.vector_search import ChunkVectorIndex
from gcb.storage.config import RawStoreConfig, RetrievalConfig
...
class GovernedContext:
    ...
    def ingest_source_records(self, records: list[SourceRecord]) -> None:
        ...
        with tracer.start_as_current_span("gcb.source_records.ingest") as span:
            span.set_attribute("gcb.source_record.count", len(records))
            span.set_attribute(
                "gcb.source_record.source_systems",
                _joined_unique(record.source_system for record in records),
            )
            span.set_attribute(
                "gcb.source_record.record_types",
                _joined_unique(record.record_type for record in records),
            )
            try:
                self.apply_source_changes(upsert_source_records(records))
            except Exception as exc:
                span.set_attribute("gcb.source_record.status", "failed")
                span.set_attribute("gcb.error.class", type(exc).__name__)
                raise
            span.set_attribute("gcb.source_record.status", "succeeded")
    ...
    def search_context(
        ...
        *,
        ...
        with tracer.start_as_current_span("gcb.search_context") as span:
            try:
                ...
                record_counter("gcb_search_requests_total", attributes={"status": "failed"})
                record_histogram("gcb_search_duration_seconds", perf_counter() - started)
                raise
            record_counter("gcb_search_requests_total", attributes={"status": "succeeded"})
            record_histogram("gcb_search_duration_seconds", perf_counter() - started)
            record_histogram("gcb_search_results", len(response.results))
            return response
    ...
    def _search_context_with_span(
        ...
        *,
        ...
        span.set_attribute("gcb.search.limit", limit)
        span.set_attribute("gcb.search.query_length", len(query))
        ...
        span.set_attribute("gcb.search.denied_source_count", len(denied_refs))
        ...
        span.set_attribute("gcb.search.result_count", len(results))
        span.set_attribute("gcb.search.audit_recorded", bool(audit_ref))
        ...
        span.set_attribute(
            "gcb.search.evidence_item_count",
