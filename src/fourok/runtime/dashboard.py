...
def operator_dashboard(
    ...
    *,
    ...
    with tracer.start_as_current_span("fourok.dashboard") as span:
        ...
        span.set_attribute(
            "fourok.dashboard.source_record_count",
            report["source_records"]["total"],
        )
        span.set_attribute(
            "fourok.dashboard.connector_job_count",
            report["connectors"]["jobs"]["total"],
        )
        span.set_attribute(
            "fourok.dashboard.webhook_backlog_count",
            report["webhooks"]["by_status"].get("pending", 0),
        )
        span.set_attribute(
            "fourok.dashboard.slack_message_count",
            report["slack_messages"]["active_total"],
        )
        span.set_attribute(
            "fourok.dashboard.audit_event_count",
            report["audit"]["total_events"],
        )
        span.set_attribute(
            "fourok.dashboard.alert_count",
            len(report["alerts"]["items"]),
        )
        span.set_attribute("fourok.dashboard.alert_status", report["alerts"]["status"])
        return report
...
def _alert_summary(
    *,
    ...
    _append_alert(
        ...
        next_step=(
            "Run `fourok webhook-events --status pending`, then process due events "
            "with `fourok webhook-process`."
        ),
    )
    _append_alert(
        items,
        ...
        next_step=(
            "Run `fourok webhook-events --status failed`, inspect the error class, "
    ...
    _append_alert(
        ...
        next_step=(
            "Run `fourok webhook-events --status invalid`, inspect the raw payload "
