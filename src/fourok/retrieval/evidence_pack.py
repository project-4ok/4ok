...
from fourok.observability import record_histogram
...
def build_evidence_pack(
    *,
    ...
    with tracer.start_as_current_span("fourok.evidence_pack.build") as span:
        ...
        span.set_attribute("fourok.evidence_pack.result_count", len(results))
        span.set_attribute("fourok.evidence_pack.source_record_count", len(source_records))
        span.set_attribute("fourok.evidence_pack.canonical_object_count", len(canonical_objects))
        span.set_attribute("fourok.evidence_pack.entity_link_count", len(entity_links or []))
        span.set_attribute(
            "fourok.evidence_pack.evidence_item_count",
            len(pack.get("evidence_items", [])),
        )
        span.set_attribute(
            "fourok.evidence_pack.related_object_count",
            len(pack.get("related_objects", [])),
        )
        span.set_attribute(
            "fourok.evidence_pack.unresolved_candidate_count",
            len(pack.get("unresolved_candidates", [])),
        )
        span.set_attribute(
            "fourok.evidence_pack.limitation_count",
            len(pack.get("limitations", [])),
        )
        record_histogram("fourok_evidence_pack_items", len(pack.get("evidence_items", [])))
        record_histogram("fourok_evidence_pack_limitations", len(pack.get("limitations", [])))
        return pack
