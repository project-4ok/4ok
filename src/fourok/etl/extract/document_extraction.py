...
def extract_document(
    ...
    *,
    ...
    with critical_span(
        "fourok.document.extract",
        attributes={
            "fourok.document.extractor": "docling",
            "fourok.document.extension": document_path.suffix.lower(),
        },
        status_attribute="fourok.document.status",
    ) as span:
        try:
        ...
        set_safe_span_attributes(
            span,
            {
                "fourok.document.status": "succeeded",
                "fourok.document.content_type": extracted.content_type,
                "fourok.document.text_length": len(extracted.text),
            },
        )
        return extracted
...
def land_raw_pdf(path: Path | str, landing_dir: Path | str) -> LandedPdf:
    with critical_span(
        "fourok.raw_landing.document",
        attributes={"fourok.document.extension": Path(path).suffix.lower()},
