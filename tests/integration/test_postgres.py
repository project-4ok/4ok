...
@pytest.mark.skipif(
    not os.environ.get("FOUROK_TEST_DATABASE_URL"),
    reason="set FOUROK_TEST_DATABASE_URL to run PostgreSQL integration tests",
)
def test_postgres_full_text_search_matches_expected_email() -> None:
    context = GovernedContext(database_url=os.environ["FOUROK_TEST_DATABASE_URL"])
